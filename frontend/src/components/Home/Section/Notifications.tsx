      ...
      {notifications.length > 0 ? (
        <ul className="space-y-2 max-h-40 overflow-y-auto">
          ...
          {notifications.length > 5 && (
            <details className="p-2 rounded bg-lightGreen-100 cursor-pointer">
              <summary className="text-green-600 underline">View more</summary>
              {notifications.slice(5).map((notification) => (
                <li key={notification.id} className={`p-2 rounded flex items-center ${notification.read ? 'bg-gray-100' : 'bg-green-200'}`}>
                  <button
                    className={`mr-2 ${notification.read ? 'text-gray-400' : 'text-green-600'}`}
                    onClick={() => markAsRead(notification.id)}
                    title={notification.read ? "Marked as read" : "Mark as read"}
